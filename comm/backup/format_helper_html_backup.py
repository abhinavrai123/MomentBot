...
def format_section_1_journal_html(logs):
    ...
    for log in logs:
        ...
        entry = f"<div class='entry-block'>• <i>[{time_str}]</i> {energy} {comment}</div>"
...
def format_section_2_wins_gratitude_html(logs):
    ...
    for log in logs:
        ...
        comment = wrap_text(log.comment or "")
        entry = f"<div class='entry-block'>• <i>[{time_str}]</i> {emoji} {energy} {comment} <i>(Cog: {cog}, Tag: {tag}, Trg: {trg})</i></div>"
        lines.append(entry)
...
def format_section_3_reflection_html(logs):
    ...
    for log in logs:
        ...
        entry = f"<div class='entry-block'>• <i>[{time_str}]</i> {energy} {comment} <i>(Cog: {cog}, Tag: {tag}, Trg: {trg})</i></div>"
        routine_blocks[label].append(entry)
...
<html>
  ...
  <body>
    <div class="email-container">
      <div style="font-size: 16px; color: #666; text-align: center; margin-bottom: 24px;">
